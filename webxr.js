(function () {
  ...
  XRManager.prototype.keyUp = function (evt) {
    ...
    // performance hud
    if (evt.key == "p") {
      this.perfStatus.dataset.enabled =
        this.perfStatus.dataset.enabled === "true" ? false : true;
    }
  };
  ...
  XRManager.prototype.setGameInstance = function (gameInstance) {
    if (!this.gameInstance) {
      ...
      this.gameInstance.Module.InternalBrowser.requestAnimationFrame = function (
        func
      ) {
        ...
        if (thisXRMananger.vrSession && thisXRMananger.isInVRSession) {
          return thisXRMananger.vrSession.requestAnimationFrame(
            (time, xrFrame) => {
          ...
          return thisXRMananger.inlineSession.requestAnimationFrame(
            (time, xrFrame) => {
              if (func) {
                func(time);
              }
    ...
    for (let source of frame.session.inputSources) {
      if (source.gripSpace && source.gamepad) {
        ...
        gamepads.push({
          ...
          hasOrientation: true,
          hasPosition: true,
          orientation: this.GLQuaternionToUnity([
            orientation.x,
            orientation.y,
            orientation.z,
            orientation.w,
  ...
  XRManager.prototype.onSessionStarted = function (session) {
    ...
    session.requestReferenceSpace(refSpaceType).then((refSpace) => {
      if (session.isImmersive) {
        ...
        this.gameInstance.Module.InternalBrowser.requestAnimationFrame(
          this.rAFCB
        );
      } else {
  ...
  XRManager.prototype.animate = function (t, frame) {
    ...
    for (let view of pose.views) {
      if (view.eye === "left") {
        xrData.leftProjectionMatrix = this.GLProjectionToUnity(
          view.projectionMatrix
        );
        xrData.leftViewMatrix = this.GLViewToUnity(
          view.transform.inverse.matrix
        );
      } else if (view.eye === "right") {
        xrData.rightProjectionMatrix = this.GLProjectionToUnity(
          view.projectionMatrix
        );
        xrData.rightViewMatrix = this.GLViewToUnity(
          view.transform.inverse.matrix
        );
      } else {
        xrData.sitStandMatrix = this.GLViewToUnity(
          view.transform.inverse.matrix
        );
      }
    }
